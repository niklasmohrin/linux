...
unsafe extern "C" fn write_begin_callback<T: AddressSpaceOperations>(
    ...
    unsafe {
        ...
        from_kernel_result! {
            a_ops.write_begin(file.as_deref(), mapping.as_mut().expect("Got null mapping").as_mut(), pos, len, flags, pagep, fsdata).map(|()| 0)
        }
    }
}
...
unsafe extern "C" fn write_end_callback<T: AddressSpaceOperations>(
    ...
    unsafe {
        ...
        let file = (!file.is_null()).then(|| FileRef::from_ptr(file));
        from_kernel_result! {
            a_ops.write_end(file.as_deref(), mapping.as_mut().expect("Got null mapping").as_mut(), pos, len, copied, &mut (*page), fsdata).map(|x| x as i32)
        }
    }
}
